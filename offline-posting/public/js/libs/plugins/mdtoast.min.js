 ...
 * -- DO NOT REMOVE --
 */
if (typeof jQuery === 'undefined') { throw new Error('MDToast: This plugin requires jQuery') } + function($) {
    var MDToast = function(message, options) {
        ...
        this.toast.addClass(options.type === 'default' ? '' : 'mdt--' + options.type).append('<div class="mdt-message">' + message + '</div>');
        if (options.interaction) { var that = this;
            this.action.text(options.actionText).on('click', function() { if (options.action) options.action(that) });
    ...
    MDToast.prototype = {
        ...
        show: function() {
            var that = this,
                ...
                existingToast = $('.mdtoast'),
                doc = $('body');
            if (that.toast.is(':visible')) return;
            that.toast.data(that.TOAST_DATA, that).appendTo(doc);
            setTimeout(function() {
                that.toast.removeClass('mdt--load');
                if (existingToast.length > 0) { existingToast.each(function() { var ex_toast = $(this).data(that.TOAST_DATA);
                        ex_toast.hide() }) }
                ...
                } else if (that.options.duration) { that.toastTimeout = setTimeout(function() { that.hide() }, that.options.duration) }
        ...
        hide: function() { var that = this,
            ...
            setTimeout(function() { that.toast.remove(); if (callbacks && callbacks.hidden) callbacks.hidden() }, that.animateTime) }
    }
    $.mdtoast = function(message, options) { return new MDToast(message, $.extend({}, $.mdtoast.defaults, null, typeof options === 'object' && options)) }
    $.mdtoast.defaults = { init: !1, duration: 5000, type: 'default', modal: !1, interaction: !1, interactionTimeout: null, actionText: 'OK', action: function(data) { data.hide() }, callbacks: {} };
    $.mdtoast.type = { INFO: 'info', ERROR: 'error', WARNING: 'warning', SUCCESS: 'success' }
}(jQuery)